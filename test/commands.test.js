...
test('exports the expected slash command manifest', () => {
  assert.equal(commandJson.length, 6);

  const [download, watch, profiles, status, history, downloads] = commandJson;
  ...
  assert.equal(profiles.name, 'profiles');
  assert.deepEqual((profiles.options ?? []).map((option) => option.name), ['link', 'show', 'unlink']);
  assert.deepEqual((profiles.options[0].options ?? []).map((option) => option.name), ['primary', 'secondary', 'name', 'merge']);
  assert.equal((profiles.options[0].options ?? [])[0].required, true);
  assert.equal((profiles.options[0].options ?? [])[1].required, true);
  assert.equal((profiles.options[0].options ?? [])[2].required, false);
  assert.equal((profiles.options[0].options ?? [])[3].required, false);
  assert.deepEqual((profiles.options[1].options ?? []).map((option) => option.name), ['profile']);
  assert.equal((profiles.options[1].options ?? [])[0].required, true);
  assert.deepEqual((profiles.options[2].options ?? []).map((option) => option.name), ['profile']);
  assert.equal((profiles.options[2].options ?? [])[0].required, true);

  assert.equal(status.name, 'status');
  assert.match(status.description, /TikTok monitor/);
  ...
  assert.equal((downloads.options[0].options ?? [])[1].description, 'Filter by creator username.');
